...
configurations <- list(
                        ...
                        list(ylags=5, zlags=5, xlags=6, boxcox=TRUE, boxcoxL=-0.5, hashrate_almonlag=1, btcclose_almonlag=1, smoothed=TRUE),
                        list(ylags=5, zlags=5, xlags=6, boxcox=TRUE, boxcoxL=-1, hashrate_almonlag=1, btcclose_almonlag=1, smoothed=TRUE),
                        list(ylags=5, zlags=5, xlags=6, boxcox=TRUE, boxcoxL=0, hashrate_almonlag=1, btcclose_almonlag=1, smoothed=TRUE),
...
for (config in configurations) {
    ...
    if (btcclose_almonlag==1){
      winsizes <- c(1260)  # c(720, 900, 1080, 1260)
    } else {
      winsizes <- c(1080)  # c(720, 900, 1080, 1260)
    }

    for (window_size in winsizes){

          if (boxCox) {
            DIR_out <- sprintf("%scasestudy2/casestudy2_updfits2/rolling_fits_ylags_%s_zlags_%s_xlags_%s_hashrateAlmon_%s_btccloseAlmon_%s_boxCox_%s_lamda_%s_step_%s_win_%s/", DIR, ylags,
                               zlags, xlags, hashrate_almonlag, btcclose_almonlag, boxCox, boxCox_lamda, stepsize, window_size)
          } else {
            DIR_out <- sprintf("%scasestudy2/casestudy2_updfits2/rolling_fits_ylags_%s_zlags_%s_xlags_%s_hashrateAlmon_%s_btccloseAlmon_%s_boxCox_%s_step_%s_win_%s/", DIR, ylags,
          ...
          coef_ylag1 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          coef_ylag2 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          coef_ylag3 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          coef_ylag4 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          coef_ylag5 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          ...
          ######################################################################
          error_ylag1 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          error_ylag2 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          error_ylag3 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          error_ylag4 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          error_ylag5 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          ...
          ######################################################################
          significance_ylag1 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          significance_ylag2 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          significance_ylag3 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          significance_ylag4 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          significance_ylag5 <- rep(-1000, 1 + floor((length(y)-window_size)/stepsize))
          ...
          xxvals <- matrix(-1000, nrow = length(y) , ncol=(1 + floor((length(y)-window_size)/stepsize)))
          zzvals <- matrix(-1000, nrow = (24*length(y)) , ncol=(1 + floor((length(y)-window_size)/stepsize)))
          yyvals <- matrix(-1000, nrow = length(y) , ncol=(1 + floor((length(y)-window_size)/stepsize)))


          y_pvalues <- matrix(-1000, nrow = ylags , ncol=(1 + floor((length(y)-window_size)/stepsize)))
          ...
          model_sd <- rep(100000, 1 + floor((length(y)-window_size)/stepsize))
          ...
          for (i in seq(1, length(y), stepsize) ) {
            ...
            model_sd[k] <- summary(eqr)$sigma
            mse <- mean((fitted(eqr) - yy[(xlags*30+1):length(yy)])^2)
            aic <- AIC(eqr)

            coef_ylag1[k] <- coef(eqr, midas=FALSE, "yy")[1]
            coef_ylag2[k] <- coef(eqr, midas=FALSE, "yy")[2]
            coef_ylag3[k] <- coef(eqr, midas=FALSE, "yy")[3]
            coef_ylag4[k] <- coef(eqr, midas=FALSE, "yy")[4]
            coef_ylag5[k] <- coef(eqr, midas=FALSE, "yy")[5]

            y_pvalues[, k] <- summ[2:(ylags+1), 4]

            z_theta1[k] <- coef(eqr, midas=FALSE, "zz")[1]
            z_almonlag1[k] <- coef(eqr, midas=FALSE, "zz")[2]

            pvalues_z_theta1[k] <- summ[1+ylags+1, 4] # MIDAS false
            pvalues_z_almonlag1[k] <- summ[1+ylags+1+1, 4]
            if (btcclose_almonlag==2){
              z_almonlag2[k] <- coef(eqr, midas=FALSE, "zz")[3]
              pvalues_z_almonlag2[k] <- summ[1+ylags+1+1+1, 4]
            }

            x_theta1[k] <- coef(eqr, midas=FALSE, "xx")[1]
            x_almon_1[k] <- coef(eqr, midas=FALSE, "xx")[2]

            pvalues_x_theta1[k] <- summ[(1+ylags+btcclose_almonlag+1+1), 4] # MIDAS false
            pvalues_x_almonlag1[k] <- summ[(1+ylags+btcclose_almonlag+1+1+1), 4]
            ...
            ######################################################################
            error_ylag1[k] <- summ[, 2][2]
            error_ylag2[k] <- summ[, 2][3]
            error_ylag3[k] <- summ[, 2][4]
            error_ylag4[k] <- summ[, 2][5]
            error_ylag5[k] <- summ[, 2][6]

            error_z_theta1[k] <- summ[, 2][7]
            error_z_almonlag1[k] <- summ[, 2][8]
            if (btcclose_almonlag==2){
              error_z_almonlag2[k] <- summ[, 2][9]
            }
            ...
            if (btcclose_almonlag==2){
              ...
              error_x_theta1[k] <- summ[, 2][9]
              error_x_almonlag1[k] <- summ[, 2][10]
            }

            error_intercepts[k] <- summ[, 2][1]

            ######################################################################
            significance_ylag1[k] <- star_significance(summ[, 4][2])
            significance_ylag2[k] <- star_significance(summ[, 4][3])
            significance_ylag3[k] <- star_significance(summ[, 4][4])
            significance_ylag4[k] <- star_significance(summ[, 4][5])
            significance_ylag5[k] <- star_significance(summ[, 4][6])

            significance_z_theta1[k] <- star_significance(summ[, 4][7])
            significance_z_almonlag1[k] <- star_significance(summ[, 4][8])

             if (btcclose_almonlag==2){
              significance_z_almonlag2[k] <- star_significance(summ[, 4][9])
             }
             ...
             if (btcclose_almonlag==2){
               ...
               significance_x_theta1[k] <- star_significance(summ[, 4][9])
               significance_x_almonlag1[k] <- star_significance(summ[, 4][10])
             }
            ...
            xxvals[(1:length(xx)), k] <- xx
            zzvals[(1:length(zz)), k] <- zz
            yyvals[(1:length(yy)), k] <- yy
          ...
          data_global <- data.frame(window_start_dates=window_start_dates,
                                    coef_ylag1=coef_ylag1,
                                    coef_ylag2=coef_ylag2,
                                    coef_ylag3=coef_ylag3,
                                    coef_ylag4=coef_ylag4,
                                    coef_ylag5=coef_ylag5,
                                    z_theta1=z_theta1,
                                    ...
                                    error_ylag1=error_ylag1,
                                    error_ylag2=error_ylag2,
                                    error_ylag3=error_ylag3,
                                    error_ylag4=error_ylag4,
                                    error_ylag5=error_ylag5,
                                    ...
                                    significance_ylag1=significance_ylag1,
                                    significance_ylag2=significance_ylag2,
                                    significance_ylag3=significance_ylag3,
                                    significance_ylag4=significance_ylag4,
                                    significance_ylag5=significance_ylag5,
          ...
          xxframe <- data.frame(xx=xxvals)
          write.csv(xxframe, sprintf("%srollingfits_xx.csv", DIR_out), row.names = FALSE)
          zzframe <- data.frame(zz=zzvals)
          write.csv(zzframe, sprintf("%srollingfits_zz.csv", DIR_out), row.names = FALSE)
          yyframe <- data.frame(yy=yyvals)
          write.csv(yyframe, sprintf("%srollingfits_yy.csv", DIR_out), row.names = FALSE)
          model_sd_fits <- data.frame(dates=window_start_dates, sd=model_sd)
          write.csv(model_sd_fits, sprintf("%srollingfits_model_standarddev.csv", DIR_out), row.names = FALSE)
